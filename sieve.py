...
# This script is paired with `sieve.ipynb` via Jupytext. Edits to either file will sync on commit.
...
def main():
    ...
    print(
        f"[sieve] backend={backend} traces={total} np_hits={total_np} ({pct:.2f}%) csv={CSV_PATH}"
    )
    return 0
